...
module.exports.checkPhotoContent = function (req, res, next) {
  ...
  clarifyModel
    .predictByBytes({ inputType: 'image', inputBytes: req.file.buffer })
    .then(
      function (response) {
      ...
      function (err) {
        ...
        res.render('upload', {
          error: true,
          resultMessage: 'Η ανάλυση της εικόνας απέτυχε',
          courses: courses.all,
          years: settings.years,
        });
      }
...
module.exports.updateItemCountForCourse = function (
  courseKey,
  year,
  incremenet
) {
  semesters
    .findOne({ 'courses.key': courseKey })
    .then(function (doc) {
      if (doc == null) {
